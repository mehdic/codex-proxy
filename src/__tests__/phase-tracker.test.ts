...
test("reports command execution as shell progress", () => {
  ...
  assert.equal(phase?.text, "[progress: using shell…]");
...
test("reports long waits once", () => {
  ...
  assert.equal(tracker.poll()?.text, "[progress: waiting for shell, 12s…]");
  assert.equal(tracker.poll(), null);
});
...
test("reports file changes as files progress", () => {
  ...
  assert.equal(tracker.poll()?.text, "[progress: using files…]");
});
